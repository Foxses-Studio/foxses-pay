...
export class PaymentGateway {
  ...
  private static registry: Map<SupportedProvider, ProviderConstructor> =
    new Map();
  ...
  use(provider: SupportedProvider, config: PaymentConfig): this {
    ...
    if (!Ctor) {
      throw new ValidationError(
        `Provider "${provider}" is not registered. Did you import it?`
      );
    }
  ...
  private getProvider(provider: SupportedProvider): BaseProvider {
    ...
    if (!p) {
      throw new ValidationError(
        `Provider "${provider}" has not been configured. Call gateway.use("${provider}", config) first.`
      );
    }
    return p;
  }
